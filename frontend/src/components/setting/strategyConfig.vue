    ...
    <v-row justify="center">
        <v-dialog v-model="dialog" fullscreen :scrim="false" transition="dialog-bottom-transition">
            <template v-slot:activator="{ props }">
                <v-autocomplete label="策略配置" v-bind="props" style="padding: 0 25px;" :items="[]"
            ...
            <v-card>
                <v-toolbar dark color="primary">
                    ...
                    <v-toolbar-title>策略配置</v-toolbar-title>
                    <v-spacer></v-spacer>
                </v-toolbar>
                <v-list lines="two" subheader>
                    ...
                    <div class="default-config">
                        ...
                        <div class="sConfig-exchange">
                            <!-- <v-select v-model="strategyConfig.nowExhangesData.exchange" class="se-select" clearable
                                label="交易所" :items="State.AltExchanges" variant="underlined"></v-select> -->
                            <v-responsive class="mx-auto">
                                <v-text-field v-model="strategyConfig.nowExhangesData.exchange" clearable
                                    hide-details="auto" label="交易所" variant="underlined"></v-text-field>
                            </v-responsive>
                            <v-responsive class="mx-auto">
                                <v-text-field v-model="strategyConfig.nowExhangesData.chip" clearable
                                    hide-details="auto" label="交易对" variant="underlined"
                                    hint="请使用XXX_XXX格式"></v-text-field>
                            </v-responsive>
                            <v-btn
                        ...
                        <div class="Exchanges">
                            <v-chip v-for="(name, index) in upconfig.Exchanges" @click="closeChip(index)"
                                variant="elevated" class="Exchanges-chips">
                                {{ name.join(' / ') }}
                            </v-chip>
                        </div>
                    </div>
                    ...
                    <div class="senior-config">
                        <div :key="index" v-for="(i, index) in upconfig.CusConfig" style="margin-bottom: 10px;">
                            <v-responsive v-if="i.type == 'number'" class="mx-auto">
                                <v-text-field v-model="i.default" clearable hide-details="auto" :label="i.name"
                                    :hint="i.remarks" placeholder="number类型" variant="underlined"></v-text-field>
                            </v-responsive>
                            <v-responsive v-else-if="i.type == 'string'" class="mx-auto">
                                <v-text-field v-model="i.default" clearable hide-details="auto" :label="i.name"
                                    :hint="i.remarks" placeholder="string类型" variant="underlined"></v-text-field>
                            </v-responsive>
                            <v-select v-else-if="i.type == 'select'" v-model="i.default" :label="i.name"
                                :items="i.items" variant="underlined" :hint="i.remarks"></v-select>
                            ...
                            <div v-else-if="i.type == 'slider'">
                                ...
                                <v-slider color="primary" v-model="i.default" class="align-center" :max="i?.max"
                                    :min="i?.min" :step="i?.step"
                                    :show-ticks="i?.step == undefined ? undefined : 'always'" hide-details>
                                    <template v-slot:append>
                                        <v-text-field v-model="i.default" hide-details single-line density="compact"
